...
function SimTimeStep(
    ...
    // Initialize total forces on each particle with gravity
    let forces = positions.map(() => gravity.mul(particleMass));

    // Compute spring and damping forces for each spring
    for (let s of springs) {
        ...
        let forceDir = dir.unit();

        let fSpring = forceDir.mul(stiffness * (dist - restLength));
        let relVel = v1.sub(v0);
        let fDamp = forceDir.mul(damping * relVel.dot(forceDir));
    ...
    // Integrate motion: update velocity and position of each particle
    for (let i = 0; i < positions.length; i++) {
        let acc = forces[i].div(particleMass);
        velocities[i].inc(acc.mul(dt));
        positions[i].inc(velocities[i].mul(dt));

        // Handle collisions with the bounding box [-1, 1] in x, y, z
        if (positions[i].x < -1) {
            positions[i].x = -1;
            if (velocities[i].x < 0)
